...
const Stars: React.FC<{ rating: number; size?: number }> = ({ rating, size = 14 }) => {
  return (
    <div style={{ display:'flex', gap:2 }}>
      {[1,2,3,4,5].map(star => (
        <span key={star} style={{ color: star <= rating ? '#F59E0B' : '#D1D5DB', fontSize:size }}>
          ★
        </span>
      ))}
    </div>
...
const AppointmentHistory: React.FC<AppointmentHistoryProps> = ({ onUnauthorized }) => {
  ...
  const AppointmentRow: React.FC<{ appointment: Appointment }> = ({ appointment: apt }) => {
    ...
    const hasFeedback = apt.feedback !== null;
      ...
      <tr style={{ background: isHov ? '#F9FAFB' : 'white', transition:'background 0.1s' }}
        ...
        <td style={TD}>
          {apt.status === 'completed' ? (
            hasFeedback ? (
              <div>
                <Stars rating={apt.feedback!.rating} size={12} />
                {apt.feedback!.comment && (
                  <div style={{ fontSize:11, color:'#6B7280', marginTop:4, maxWidth:180 }}>
                    "{apt.feedback!.comment.length > 50 ? apt.feedback!.comment.substring(0, 50) + '...' : apt.feedback!.comment}"
                  </div>
                )}
              </div>
            ) : (
              ...
              <table style={{ width:'100%', minWidth:1000, borderCollapse:'collapse' }}>
                <thead>
                  <tr style={{ background:'#F9FAFB', borderBottom:'1px solid #E5E7EB' }}>
                    {['Service','Customer','Date & Time','Staff','Status','Feedback','Actions'].map(h => (
